...
function trendFromSeries(series) {
  if (!series.length) return { trend: "out", trendLabel: "랭킹 밖", growthRate: null };
  if (series.length === 1) return { trend: "new", trendLabel: "신규", growthRate: 100 };
...
function reasonFor(article) {
  ...
  if (article.trend === "up") {
    return "최근 3시간 VIEW 흐름이 상승 중입니다. 현재 배치 위치보다 더 높은 노출을 검토할 수 있습니다.";
  }
  if (article.trend === "down") {
    return "최근 3시간 VIEW 흐름이 하락 중입니다. 상단 유지 필요성을 다시 확인하는 편이 좋습니다.";
  }
  if (article.trend === "out") {
    return "네이버 언론사별 기사 랭킹 50위 안에서 확인되지 않았습니다.";
  }
  return "최근 3시간 VIEW 흐름이 급격하게 변하지 않았습니다.";
}
...
export function mergeDashboardData(mainArticles, rankings) {
  ...
  for (const rankingWindow of rankings) {
  ...
  const maxCount = Math.max(
    1,
    ...[...seriesByCrc.values()].map((series) => series.at(-1)?.count || 0)
  );

  function enrich(article) {
    const key = article.crc32 || article.articleId;
    const series = seriesByCrc.get(key) || [];
    ...
    const enriched = {
      ...
      ...article,
      series,
      currentRank: current?.rank || meta.currentRank || null,
      latestCount: current?.count || meta.latestCount || null,
      ...trend
    };
  ...
  const externalCandidates = [...rankingMeta.entries()]
    .filter(([crc32]) => !mainCrcSet.has(crc32))
    .map(([crc32, meta], index) =>
      enrich({
        id: `external-${crc32}`,
        cntnCd: crc32,
        crc32,
        title: meta.title || `네이버 랭킹 기사 ${index + 1}`,
        url: "",
        naverUrl: meta.naverUrl,
        position: null,
        placement: "external",
  ...
  const recommendations = [...enrichedMain, ...externalCandidates]
