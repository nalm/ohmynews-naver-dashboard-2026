...
export function getRecentRankingWindows(now = new Date(), count = 3) {
  ...
  let endHour = kstMinute > 5 ? kstHour : kstHour - 1;
...
export async function fetchPublisherRankings(windows = getRecentRankingWindows()) {
  ...
  for (const rankingWindow of windows) {
    ...
    const items = data?.result?.elementList || [];
    results.push({
      ...rankingWindow,
      items: items.map(normalizeRankingItem).filter((item) => item.crc32 || item.articleId)
    });
  }
